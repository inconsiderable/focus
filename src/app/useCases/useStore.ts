  ...
  persist(
    (set, get) => ({
      ...
      setSelectedKey: (selectedKey: string) => {
        const selectedKeyIndex = findIndex(get().publicKeys, selectedKey);
        set(() => ({
          selectedKey,
          selectedKeyIndex,
        }));
...
function findIndex(array: string[][], target: string): [number, number] {
  for (let row = 0; row < array.length; row++) {
    const col = array[row].indexOf(target);
    if (col !== -1) {
      return [row, col];
    }
  }
